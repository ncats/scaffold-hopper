...
import gov.nih.ncgc.util.SMIRKS;
...
public class MolecularFramework {
    ...
    static {
        try {
            debug = Boolean.getBoolean("framework.debug");
        }
        catch (Exception ex) {
        }
    }
    ...
    protected Molecule addFragment (Molecule f) {
        ...
        String[] hk = MolStandardizer.hashKeyArrayExt(f);
                    ...
                    */
                    ...
                    for (Enumeration<Molecule> en = mf.getFragments();
                        ...
                        /*
                          ...
                          +"\t"+MolStandardizer.hashKey(f));
                        */
                        //f.setName(mol.getName()+"."+j);
                        String hashkey = MolStandardizer.hashKeyExt(f);
                        f.setProperty("HashKey", hashkey);
                        f.setProperty
                            ("SMILES", MolStandardizer.canonicalSMILES(f));
                        f.setProperty("MAP", f.toFormat("smiles:q0"));
                        f.setProperty("PARENT", mol.getName());
                        f.setProperty("SCORE", String.format("%1$.3f", m));
                        //System.out.print(f.toFormat("sdf"));
                        /*
                        System.out.println(MolStandardizer.canonicalSMILES(f)
                                           +"\t" + mol.getName()
                                           +"\t"+hashkey);
                        */
