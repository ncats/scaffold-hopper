// $Id: MolStandardizer.java 3910 2010-01-12 20:08:10Z nguyenda $
 ...
 *     fixed NormalizedRules 0 & 35.  TautomerGenerator was modified
 *     to return immediately if count == 0.
 *
...
public class MolStandardizer {
     ...
     */
    public static final int VERSION = 0x08;
    ...
    static {
	try {
	    debug = Boolean.getBoolean("standardizer.debug");
	}
	catch (Exception ex) {
	}
    }
    ...
    public MolStandardizer () {
	init (new TautomerGenerator ());
    }

    public MolStandardizer (TautomerGenerator taugen) {
	init (taugen);
    }
	...
	this.taugen = taugen;
	//taugen.set(TautomerGenerator.FLAG_ALL);
	// our transform rules take care of this already
	//taugen.unset(TautomerGenerator.FLAG_NOXIDE);
    }
	...
	Vector<MolBond> bonds = new Vector<MolBond>();
    ...
    static void adjustEZStereo (Molecule mol) {
	for (MolBond b : mol.getBondArray()) {
	    if (b.getType() == 2) {
	...
	fixRadical (mol);
	if (debug) {
	    System.err.println("fixRadical: " + canonicalSMILES (mol)
			       //+" "+mol.toFormat("smiles:q"));
			       +"\n"+mol.toFormat("mol"));
	}
	...
	fixValence (mol);
	if (debug) {
	    System.err.println("fixValence: " + canonicalSMILES (mol) 
			       //+ " " + mol.toFormat("smiles:q"));
			       +"\n"+mol.toFormat("mol"));
	}
	...
	kekulize (mol);
	if (debug) {
	    System.err.println("kekulize: " + canonicalSMILES (mol) 
			       //+ " " + mol.toFormat("smiles:q"));
			       +"\n"+mol.toFormat("mol"));
	}
	...
	protonate (mol);
	if (debug) {
	System.err.println("neutralize: " + canonicalSMILES (mol));
	}
	*/
	...
	/*
	  deprotonate (mol);
	  if (debug) {
	  System.err.println("deprotonate: " + canonicalSMILES (mol) 
	  + " " + mol.toFormat("smiles:q"));
	  }
	*/
	...
	for (int i = 0; i < frags.length; ++i) {
		...
		if (f.getAtomCount() > 3) {
			...
			for (Enumeration<Molecule> tau = taugen.getTautomers();
			     tau.hasMoreElements(); ++cnt) {
			    Molecule t = tau.nextElement();
			    int score = TautomerGenerator.scoreTautomer(t);
			    System.err.println(canonicalSMILES (t) + " "+cnt
					       + " " + score);
			}
	...
	if (frags.length > 1) {
	    Arrays.sort(frags, new MolComparator ());
            ...
            List<Molecule> notnulls = new ArrayList<Molecule>();
		...
		if (frags[i] != null && frags[i].getAtomCount() > 0) {
		    ...
		    //fused.append(canonicalSMILES (frags[i]));
		    // can't use canonicalSMILES here since we'll loose the
		    //  atom mappings
		    fused.append(frags[i].toFormat("smiles:q"));
                    notnulls.add(frags[i]);
		}
	    }
		...
		if (!MolImporter.importMol(fused.toString(), mol)) {
            ...
            // only keep non null components
            frags = notnulls.toArray(new Molecule[0]);
	...
	if (debug) {
	    System.err.println("Postprocessing: " + canonicalSMILES (mol)
			       + " "+mol.toFormat("smiles:q")
			       +"\n"+mol.toFormat("mol"));
	}
	...
	if (debug) {
	    System.err.println("Stereo Adjustment: " + canonicalSMILES (mol)
			       //+ " "+mol.toFormat("smiles:q"));
			       +"\n"+mol.toFormat("mol"));
	}
	...
	cleanUp (aflags, mol);
	if (debug) {
	    System.err.println("CleanUp: " + canonicalSMILES (mol)
			       //+ " "+mol.toFormat("smiles:q"));
			       +"\n"+mol.toFormat("mol"));
	}
	...
	if (neg > 0) {
		...
		System.err.println(canonicalSMILES (mol));
	    }
	}
    ...
    public static String hash34 (String str) {
	return hashStr (str, 34);
    }
    public static String hash51 (String str) {
	return hashStr (str, 51);
    }
    public static String hash17 (String str) {
	return hashStr (str, 17);
    }

    public static String hash15 (String str) {
	return hashStr (str, 15);
    }
    public static String hash30 (String str) {
	return hashStr (str, 30);
    }
    public static String hash45 (String str) {
	return hashStr (str, 30);
    }

    protected static String hashStr (String str, int size) {
	String key = "";
	try {
	    ...
	    switch (size) {
            case 15:
                key = Base32.encode15(h);
                break;

            case 30:
                key = Base32.encode30(h);
                break;

            case 45:
                key = Base32.encode45(h);
                break;

	    case 34:
		key = Base52.encode34(h);
		break;

	    case 51:
		key = Base52.encode51(h);
		break;

	    case 17:
		key = Base52.encode17(h);
		break;

	    default:
		throw new IllegalArgumentException 
		    ("Unsupported hash size: " + size);
	    }
	}
	catch (Exception ex) {
	    logger.log(Level.SEVERE, 
		       "Can't encode utf-8 string with SHA1 hash", ex);
	}
	return key;
    }
    ...
    public static String hashKeyExt (Molecule mol) {
	return hashKeyExt (mol, "-");
    }

    public static String hashKey (Molecule mol, String sep) {
	String[] keys = hashKeyArray (mol);
	return keys[0]+sep+keys[1]+sep+keys[2];
    }

    public static String hashKeyExt (Molecule mol, String sep) {
	String[] keys = hashKeyArrayExt (mol);
	return keys[0]+sep+keys[1]+sep+keys[2]+sep+keys[3];
    }

    public static String[] hashKeyArrayOld (Molecule mol) {
	if (mol.getAtomCount() == 0) {
	    return new String[]{hash51 (""), hash34 ("") , hash17 ("")};
	}

	String cansmi = canonicalSMILES (mol, false);
	String smi1 = clearIsotopes (cansmi);

	Molecule m = null;
	try {
	    m = MolImporter.importMol(smi1);
	}
	catch (chemaxon.formats.MolFormatException ex) {
	    logger.log(Level.WARNING, 
		       "Failed to clear isotopes for " +cansmi
		       +"; reverting back", ex);
	    smi1 = cansmi;
	}

	String smi2 = canonicalSMILES (mol, true);
	// generate skeleton...
	for (MolAtom a : m.getAtomArray()) {
	    a.setAtno(6);
	    a.setRadical(0);
	...
	String smi0 = m.toFormat("smiles:q0");

	String key0 = hash51 (smi0); // truncated hash key
	String key1 = hash34 (smi0+smi1);
	String key2 = hash17 (smi0+smi2);

	return new String[]{key0, key1, key2};
    }

    public static String[] hashKeyArray (Molecule mol) {
	if (mol.getAtomCount() == 0) {
	    return new String[]{hash34 (""), hash34 ("") , hash34 ("")};
	}
	...
	String level0 = canonicalSMILES (m0, false);
	...
	String level2 = "";
	try {
	    level2 = canonicalSMILES (mol, true);
	}
	catch (Exception ex) {
	}

	// now generate hash keys
	String key0 = hash34 (level0);
	String key1 = hash34 (level1+level0);
	String key2 = hash34 (level2+level1+level0);

	if (debug) {
	    logger.info("hash layers:\n"+
			"0: "+level0 + " => " + key0 + "\n"+
			"1: "+level1+level0+" => "  + key1+"\n"+
			"2: "+level2+level1+level0+ " => " + key2);
	}
     ...
     *  key.
     */
    public static String[] hashKeyArrayExt (Molecule input) {
	if (input == null || input.getAtomCount() == 0) {
	    return hashChain45 ("", "", "", "");
	}

	Molecule mol = null;
	String molstr = canonicalSMILES (input);
	try {
	...
	MolStandardizer msz = new MolStandardizer (tg);
	...
	for (Molecule mol = new Molecule (); ; ) {
		...
		*/
		...
		String smi = canonicalSMILES (mol, true);
		if (dim > 0) {
		    mol.setProperty("Std_SMILES", smi);
		    mol.setProperty("HashKey", hashKeyExt (mol));
			...
			for (Molecule frag : msz.getFragments()) {
                            os.println(canonicalSMILES (frag) + "\t" 
                                       + name + "\t" + hashKeyExt (frag)); 
			}
		    }
		}
		/*
